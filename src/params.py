...
def get_default_params(model_name):
    ...
    elif model_name in ["ViT-B-32", "ViT-L/14", "ViT-B/16", "ViT-L-14"]:
...
def parse_args():
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.01, help="hard mix"
    )
    parser.add_argument(
        "--topk", type=int, default=10, help="hard mix"
    )
    parser.add_argument(
        "--stage", type=int, default=1, help="hard mix"
    )
    parser.add_argument(
        "--ca_layers", 
        type=parse_list,  # 使用自定义函数解析输入
        default=[1,3,5,7,9,11],  # 默认值
        help="Comma separated list of adapter layers"
    )
    parser.add_argument(
        "--va_layers", 
        type=parse_list,  # 使用自定义函数解析输入
        default=[3,7,9,11],  # 默认值
    )
    parser.add_argument(
        "--lora_rank", type=int, default=8, help="epoch"
    )
    parser.add_argument(
        "--ft_dataset", type=str, default='transAgg', help="epoch", choices=['transAgg', 'compodiff'],
    )
    parser.add_argument(
        "--epoch", type=int, default=1, help="epoch"
    )
    parser.add_argument(
        "--loss_ref_txt", type=float, default=1.0, help="The weight of consistency_loss"
    )
    parser.add_argument(
        "--adapter_dim", type=int, default=128, help="The weight of consistency_loss"
    )
    parser.add_argument(
        "--seed", type=int, default=1, help="The weight of consistency_loss"
    )
    parser.add_argument(
        "--loss-consistency", type=float, default=0.2, help="The weight of consistency_loss"
    )
    ...
    parser.add_argument("--eval-mode",
        type=str,
        choices=["coco", "cirr", "cirr_test", "fashion", "imgnet", "circo", "circo_test"],
        default="coco",
        help="Evaluate Pacs")
    ...
    # 使用 os.path.splitext 来分割文件名和扩展名
