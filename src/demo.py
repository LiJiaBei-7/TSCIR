...
from eval_utils import visualize_results, visualize_composed_results, visualize_clip_vis, visualize_results_only_token
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    lora = LoRA(original_dim=model.embed_dim, lora_rank=args.lora_rank, layer_num=6)
    ...
    if "csv" in args.retrieval_data:
        ...
        root_project = '/mnt_rela/wangyabing.wyb/datasets/Fashion-IQ'
    ...
    # visualize_clip_vis(model, img2text, args, prompt, dataloader, )
    # visualize_results_only_token(model, img2text, args, prompt, dataloader, )
    # visualize_results(model, img2text, args, prompt, dataloader, )
    visualize_composed_results(lora, model, img2text, args, prompt, dataloader, )
