...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.resume is not None:
        opt = torch.load(args.resume)["args"]
        args.va_layers = opt.va_layers

    if args.openai_pretrained:
        model, preprocess_train_strong, preprocess_train, preprocess_val = load(
            args.model, jit=False)
    else:
    ...
    try:
        ...
        if args.stage == 2:
            lora = LoRA(args=args, original_dim=model.embed_dim, lora_rank=args.lora_rank, layer_num=6)
    except:
    ...
    # total_params = sum(p.numel() for p in lora.parameters() if p.requires_grad)
    # print(f"模型可训练参数量: {total_params / 1e6:.2f}M")  # 以百万（M）为单位
    # exit()
    ...
    if not torch.cuda.is_available():
        ...
        if args.distributed:
            ...
            if args.stage == 2:
                lora = torch.nn.parallel.DistributedDataParallel(lora, device_ids=[args.gpu], find_unused_parameters=False)
        if args.dp:
    ...
    data = get_data(args, (preprocess_train_strong, preprocess_train, preprocess_val))
    exclude = lambda n : "bn" in n or "ln" in n or "bias" in n or 'logit_scale' in n
    include = lambda n : not exclude(n)
    if args.stage == 2:
        named_parameters = list(lora.named_parameters())
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.stage == 2:
            train(model, img2text, data, epoch, optimizer, scaler, scheduler, args, writer, lora=lora)
        else:
                    ...
                    torch.save(
                        {
                            ...
                            'state_dict_lora': lora.state_dict(),
